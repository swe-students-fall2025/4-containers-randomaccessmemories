	...
	try:
		...
		for attr in ("text", "transcript", "data"):
			if hasattr(resp, attr):
				...
				if attr == "data" and isinstance(val, (list, tuple)) and len(val) and isinstance(val[0], dict):
...
def transcribe(audio_bytes: bytes, *, model: Optional[str] = None) -> Optional[Dict[str, Any]]:
