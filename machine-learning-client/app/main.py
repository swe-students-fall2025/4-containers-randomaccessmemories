...
def _find_poller_callable() -> Optional[Callable[[], None]]:
	...
	"""
	candidate_names = ("process_pending", "poll_once", "run_once", "run", "process", "poll")
	try:
		...
		try:
			...
			logging.getLogger(__name__).warning("No poller module found (app.poller or poller)")
			return None
	...
	logging.getLogger(__name__).warning("No callable poller entrypoint found in app.poller")
	return None
...
def _parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--interval", type=float, default=float(DEFAULT_INTERVAL), help="Seconds between poll iterations")
