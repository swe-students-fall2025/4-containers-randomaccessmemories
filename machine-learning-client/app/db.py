...
def save_audio(data: bytes, filename: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> ObjectId:
...
def insert_transcription(record_id: ObjectId | str, text: str, confidence: Optional[float] = None) -> ObjectId:
	...
	db.recordings.update_one({"_id": _id}, {"$set": {"transcription_id": res.inserted_id}})
	return res.inserted_id


def insert_structured_note(transcription_id: ObjectId | str, note: Dict[str, Any]) -> ObjectId:
	"""Insert a structured note document and link it to the transcription."""
	db = get_db()
	_tid = ObjectId(transcription_id) if not isinstance(transcription_id, ObjectId) else transcription_id
	doc = {"transcription_id": _tid, **note}
	res = db.structured_notes.insert_one(doc)
	db.transcriptions.update_one({"_id": _tid}, {"$set": {"structured_note_id": res.inserted_id}})
	return res.inserted_id


def insert_note(recording_id: ObjectId | str, transcript: str, keywords: List[str], summary: str, action_items: Optional[List[Dict[str, Any]]] = None, language: Optional[str] = None) -> ObjectId:
	...
	"""
	db = get_db()
	_rid = ObjectId(recording_id) if not isinstance(recording_id, ObjectId) else recording_id
	doc = {
		"recording_id": _rid,
...
def set_record_error(record_id: ObjectId | str, error_message: str) -> None:
	...
	db = get_db()
	_id = ObjectId(record_id) if not isinstance(record_id, ObjectId) else record_id
	db.recordings.update_one({"_id": _id}, {"$set": {"status": "error", "error": error_message}})
...
def list_records(filter_query: Optional[Dict[str, Any]] = None, limit: int = 100) -> Iterable[Dict[str, Any]]:
