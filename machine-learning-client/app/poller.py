...
def _safe_transcribe(audio_bytes: bytes) -> Optional[Dict[str, Any]]:
	...
	try:
		...
		if isinstance(text, dict):
			return text
		return {"text": text}
	except Exception as exc:  # pragma: no cover - provider call can be environment-specific
...
def process_pending(limit: int = 10) -> int:
	...
	for doc in docs:
		...
		try:
			...
			transcription_id = db.insert_transcription(rid, stt_result.get("text"), stt_result.get("confidence"))
			...
			try:
				...
				keywords = note.get("keywords") if note and note.get("keywords") is not None else note.get("highlights") if note else []
				...
				db.insert_note(rid, transcript_text, keywords or [], summary or "", action_items or [], language=stt_result.get("language"))
				if file_id:
					db.update_record(rid, {"audio_gridfs_id": file_id, "language": stt_result.get("language")})
			except Exception:
				logger.exception("Failed to write web-app-compatible notes document for %s", rid)
